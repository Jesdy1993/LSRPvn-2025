...
new playerAnimation[MAX_PLAYERS];
...
stock IsAblePedAnimation(playerid)
{
    if( GetPVarType(playerid, "PlayerCuffed") || GetPVarType(playerid, "Injured") || GetPVarType(playerid, "IsFrozen") || GetPVarInt(playerid, "Hospital") || GetPVarType(playerid, "IsLive")) {
        SendClientMessage(playerid, COLOR_LIGHTRED, "ERROR: Ban khong the su dung hanh dong nay ngay bay gio.");
   		return 0;
	}
    if(IsPlayerInAnyVehicle(playerid) == 1)
    {
		SendClientMessage(playerid, COLOR_LIGHTRED, "ERROR: Ban khong the su dung hanh dong nay khi dang ngoi tren phuong tien.");
		return 0;
	}
	return 1;
}

stock IsAbleVehicleAnimation(playerid)
{
    if(GetPVarType(playerid, "PlayerCuffed") || GetPVarType(playerid, "Injured") || GetPVarType(playerid, "IsFrozen") || GetPVarInt(playerid, "Hospital") || GetPVarType(playerid, "IsLive")) {
   		SendClientMessage(playerid, COLOR_LIGHTRED, "ERROR: Ban khong the su dung hanh dong nay ngay bay gio.");
   		return 0;
...
{
	SendClientMessage(playerid, COLOR_GREEN, "SERVER ANIMATION:");
	SendClientMessage(playerid, COLOR_GRAD1, "SERVER: /handsup /drunk /bomb /rob /laugh /lookout /robman /crossarms /sit /siteat /hide /vomit /eat");
	SendClientMessage(playerid, COLOR_GRAD2, "SERVER: /wave /slapass /deal /taichi /crack /smoke /chat /dance /fucku /taichi /drinkwater /pedmove");
	SendClientMessage(playerid, COLOR_GRAD3, "SERVER: /checktime /sleep /blob /opendoor /wavedown /shakehand /reload /cpr /dive /showoff /box /tag");
	SendClientMessage(playerid, COLOR_GRAD4, "SERVER: /goggles /cry /dj /cheer /throw /robbed /hurt /nobreath /bar /getjiggy /fallover /rap /piss");
	SendClientMessage(playerid, COLOR_GRAD5, "SERVER: /salute /crabs /washhands /signal /stop /gesture /jerkoff /idles /lowrider /carchat /stripclub");
	SendClientMessage(playerid, COLOR_GRAD6, "SERVER: /blowjob /spank /sunbathe /kiss /snatch /sneak /copa /sexy /holdup /misc /bodypush");
	SendClientMessage(playerid, COLOR_GRAD6, "SERVER: /lowbodypush /headbutt /airkick /doorkick /leftslap /elbow /coprun /hitchhike /lean /nope");
	SendClientMessage(playerid, COLOR_GRAD6, "SERVER: /cashier /write /camera /beckon /carry /cslot /croulette /ccards /pose /swata /argue");
	SendClientMessage(playerid, COLOR_GRAD6, "SERVER: /presenta /pool /basketball /fuku /getup /phonetalk /crouchreload /lowbodypunch /fightidle");
	SendClientMessage(playerid, COLOR_GREEN, "SERVER: Su dung /stopani de dung hanh dong.");
	return 1;
}
...
CMD:seat(playerid)
{
	PlayAnim(playerid,"INT_HOUSE","LOU_In",4.0 ,0, 0 ,1, 1 ,0,0);
	return;
}
...
CMD:ccards(playerid,params[])
{
	...
	switch(strval(params))
	{
		case 1: PlayAnimEx(playerid, "CASINO", "cards_in", 4.0, 0, 0, 0, 1, 0, 1);
		case 2:	PlayAnimEx(playerid, "CASINO", "cards_loop", 4.0, 0, 0, 0, 1, 0, 1);
		case 3:	PlayAnimEx(playerid, "CASINO", "cards_lose", 4.0, 0, 0, 0, 1, 0, 1);
		case 4:	PlayAnimEx(playerid, "CASINO", "cards_pick_01", 4.0, 0, 0, 1, 0, 0, 1);
		case 5: PlayAnimEx(playerid, "CASINO", "cards_pick_02", 4.0, 0, 0, 1, 0, 0, 1);
		case 6: PlayAnimEx(playerid, "CASINO", "dealone", 4.0, 0, 0, 0, 0, 1, 1);
