...
task ProductionUpdate[300000]()
{
	...
	foreach(new i: Player)
	{
		...
		{
			SendClientMessageEx(i, COLOR_LIGHTBLUE, "Can ho tro_ Doi ngu Community Advisors co the giup do ban. (/yeucautrogiup de duoc giup do)");
		}
...
task playerTabbedLoop[1000]() {
	...
	foreach(new x: Player)
	{
		...
		// if(PlayerInfo[x][pRadio] == 1)
		// {
		// 	new radiotext[42];
		// 	format(radiotext ,sizeof(radiotext),"Tan So Radio: %d khz", PlayerInfo[x][pRadioFreq]);
		// 	PlayerTextDrawSetString(x,radio[x], radiotext);
		// 	PlayerTextDrawShow(x,radio[x]);
		// }
	
    //    HutPot(x);
		UpdateProgressHunger(x);
		if(PlayerInfo[x][pTimeAcceptDeath] > 0 ) PlayerInfo[x][pTimeAcceptDeath] --;
	...
	for(new i = 0; i < MAX_ARENAS; i++)
	{
	    if(PaintBallArena[i][pbActive] == 1)
	    {
			...
			if(PaintBallArena[i][pbTimeLeft] <= 0)
			{
			    ...
			    GivePlayerMoneyEx(winnerid,PaintBallArena[i][pbMoneyPool]);
...
task ServerHeartbeat[1000]() {
	...
	foreach(new i: Player)
	{
		if(MaskTimer[i] < gettime() && gPlayerLogged{i} && UseMask[i] != 0) {
			print("hide mask");
			UseMask[i] = 0;
		}
		if(PlayerMaxDrugTask[i] > 0 ) {
		    PlayerDrugTask[i]++;
		    if(gPlayerLogged{i} && PlayerDrugTask[i] >= PlayerMaxDrugTask[i] ) {
		...
		if(playerTabbed[i] == 0) {
			...
			if(++PlayerInfo[i][pConnectSeconds] >= 3600) {
				...
				if(PlayerInfo[i][pPayDayCode] == 0 ) {
					PlayerInfo[i][pPayDay] = 1; 
					PlayerInfo[i][pPayDayCode] = 10000 + random(99999); 
				}
				
				if(PlayerInfo[i][pInjured] == 0) SendClientTextDraw(i);
				//PayDay(i);
			}
		}
		...
		if(playerTabbed[i] == 0) {
			if(PlayerInfo[i][pJailTime] > 0 && --PlayerInfo[i][pJailTime] <= 0)
			{
				if(strfind(PlayerInfo[i][pPrisonReason], "[IC]", true) != -1 || strfind(PlayerInfo[i][pPrisonReason], "[ISOLATE]", true) != -1)
				{
					...
					PlayerInfo[i][pVW] = 0;
					SetPlayerPos(i, 1529.6000,-1691.2000,13.3828);
				}
				else
				{
				 	SetPlayerInterior(i, 0);
		...
		if(WantLawyer[i] >= 1)
		{
			...
			if(WantLawyer[i] == 1)
			{
				SendClientMessageEx(i, COLOR_LIGHTRED, "Ban co muon mot luat su_ (Su dung yes hoac no)");
			}
			WantLawyer[i] ++;
			if(WantLawyer[i] == 8)
			{
...
task ServerHeartbeatTwo[1000]() {
	...
	foreach(new i: Player)
	{
		...
		if(GetPlayerState(i) == PLAYER_STATE_DRIVER && !IsAPlane(idcar))
		{
		    ...
		    {
					format(antispeed, sizeof(antispeed), "{AA3333}AdmWarning{FFFF00}: %s (ID: %d, level %d) su dung Speed Hack, vehicle: %s, speed: %.0d.",sendername,i,PlayerInfo[i][pLevel],aVehicleNames[GetVehicleModel(GetPlayerVehicleID(i))-400],speed);
			...
			{
					format(antispeed,sizeof(antispeed),"{AA3333}AdmWarning{FFFF00}: %s (ID: %d, level: %d) co the dang su dung Surfly Car.",sendername,i,PlayerInfo[i][pLevel]);
					ABroadCast(COLOR_YELLOW, antispeed,1);
			}
        }
		if(CellTime[i] == 0 && CallCost[i] > 0)
		{
			...
			GivePlayerMoneyEx(i, -CallCost[i]);
		...
		switch(GetPlayerState(i)) {
		    case PLAYER_STATE_DRIVER: {
				...
				if(fVehicleHealth < 350.0)
				{
					...
					ShowNotifyTextMain(i,"Phuong tien da bi ~r~hu hong~w~ su dung /dichvu mechanic hoac /suaxe.",7);
